...
class Job:
    ...
    def __lt__(self, other):
        # Higher priority value means higher priority (for max heap behavior)
...
class PrintQueueManager:
    ...
    def dequeue_job(self):
        with self.lock:
            ...
            print(f"🖨️  Printing job {job.job_id} from user {job.user_id} (waited {job.waiting_time} time units)")
            return job
    ...
    def apply_priority_aging(self):
        with self.lock:
            ...
            # Convert heap to list, apply aging, then rebuild heap
            jobs = []
            while self.queue:
    ...
    def remove_expired_jobs(self):
        with self.lock:
            ...
            # Convert heap to list, filter expired, then rebuild heap
            jobs = []
            expired_jobs = []
            ...
            # Rebuild heap with non-expired jobs
            for job in jobs:
                heapq.heappush(self.queue, job)
    ...
    def handle_simultaneous_submissions(self, jobs):
        print(f"\nHandling {len(jobs)} simultaneous job submissions...")
        threads = []
    ...
    def tick(self):
        with self.lock:
            ...
            # Update waiting times for all jobs in queue
            for job in self.queue:
                job.waiting_time = self.current_time - job.submission_time

            # Apply aging and remove expired jobs
    ...
    def print_queue_snapshot(self):
        with self.lock:
            ...
            # Create a sorted copy for display (don't modify original heap)
    ...
    def print_job(self, job_id):
        # Simulate printing a job (this would interface with actual printer)
        print(f"🖨️  Printing job {job_id}...")
